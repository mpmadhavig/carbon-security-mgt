...
package org.wso2.carbon.security.mgt.keystore;

import org.apache.commons.lang.StringUtils;
import org.wso2.carbon.base.MultitenantConstants;
import org.wso2.carbon.base.ServerConfiguration;
import org.wso2.carbon.core.util.KeyStoreUtil;
import org.wso2.carbon.security.mgt.SecurityConfigException;
import org.wso2.carbon.security.mgt.SecurityConstants;
import org.wso2.carbon.security.mgt.keystore.service.KeyStoreData;
import org.wso2.carbon.security.mgt.keystore.service.CertData;
import org.wso2.carbon.security.mgt.keystore.service.CertDataDetail;
...
public class KeyStoreManagementServiceImpl implements KeyStoreManagementService {
    ...
    public X509Certificate getKeyStoreCertificate(String tenantDomain, String alias)
        ...
        if (StringUtils.isEmpty(alias)) {
            throw handleClientException(
                    SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_EMPTY_ALIAS, null);
        }
    ...
    public X509Certificate getClientCertificate(String tenantDomain, String alias) throws KeyStoreManagementException {
        ...
        try {
            ...
            throw handleServerException(
                    SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_RETRIEVE_CLIENT_TRUSTSTORE, tenantDomain, e);
        }
        ...
        if (trustStore != null) {
            try {
                ...
                throw handleServerException(
                        SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_RETRIEVE_CLIENT_TRUSTSTORE_CERTIFICATE, alias, e);
            }
        }
        return null;
    }
    ...
    public void addCertificate(String tenantDomain, String alias, String certificate)
        ...
        try {
            ...
            throw handleClientException(
                    SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_INVALID_CERTIFICATE, alias);
        }
        KeyStore keyStore;
        String certAlias;
        ...
        try {
            ...
            throw handleServerException(
                    SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_VALIDATE_CERTIFICATE, null, e);
        }
        if (isAliasExists) {
            throw handleClientException(SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_ALIAS_EXISTS, alias);
        }
        if (certAlias != null) {
            throw handleClientException(
                    SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_CERTIFICATE_EXISTS, certAlias);
        }
        try {
            keyStoreAdmin.importCertToStore(alias, certificate, keyStoreName);
        } catch (SecurityConfigException e) {
            throw handleServerException(SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_ADD_CERTIFICATE, alias, e);
        }
    }
    ...
    public void deleteCertificate(String tenantDomain, String alias) throws KeyStoreManagementException {
        ...
        try {
            ...
            if (publicCertificate.keySet().contains(alias)) {
                throw handleClientException(
                        SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_CANNOT_DELETE_TENANT_CERT, alias);
            }
            getKeyStoreAdmin(tenantDomain).removeCertFromStore(alias, getKeyStoreName(tenantDomain));
        } catch (SecurityConfigException e) {
            throw handleServerException(
                    SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_DELETE_CERTIFICATE, alias, e);
        }
    }
    ...
    private String getKeyStoreName(String tenantDomain) throws KeyStoreManagementException {
        ...
        try {
            ...
            throw handleServerException(SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_RETRIEVE_KEYSTORE, tenantDomain, e);
        }
        ...
        throw handleServerException(SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_RETRIEVE_KEYSTORE, tenantDomain);
    }
    ...
    private KeyStoreData getKeystoreData(String tenantDomain, String keyStoreName) throws KeyStoreManagementException {
        ...
        try {
            ...
            throw handleServerException(
                    SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_RETRIEVE_KEYSTORE_INFORMATION, keyStoreName, e);
        }
        return keyStoreData;
    }
    ...
    private List<String> filterAlias(List<String> aliases, String filter) throws KeyStoreManagementException {
        ...
        if (filter != null) {
            ...
            if (extractedFilter.length == 3) {
                if (StringUtils.equals(extractedFilter[0], SecurityConstants.KeyStoreMgtConstants.FILTER_FIELD_ALIAS)) {
                    String operation = extractedFilter[1];
                    String value = extractedFilter[2];
                    if (StringUtils.equals(operation, SecurityConstants.KeyStoreMgtConstants.FILTER_OPERATION_EQUALS)) {
                        aliases = aliases.stream().filter(alias -> alias.matches(value))
                                .collect(Collectors.toList());
                    } else if (StringUtils.equals(operation, SecurityConstants.KeyStoreMgtConstants.FILTER_OPERATION_STARTS_WITH)) {
                        aliases = aliases.stream().filter(alias -> alias.startsWith(value))
                                .collect(Collectors.toList());
                    } else if (StringUtils.equals(operation, SecurityConstants.KeyStoreMgtConstants.FILTER_OPERATION_ENDS_WITH)) {
                        aliases = aliases.stream().filter(alias -> alias.endsWith(value))
                                .collect(Collectors.toList());
                    } else if (StringUtils.equals(operation, SecurityConstants.KeyStoreMgtConstants.FILTER_OPERATION_CONTAINS)) {
                        aliases = aliases.stream().filter(alias -> alias.contains(value))
                                .collect(Collectors.toList());
                    } else {
                        throw handleClientException(
                                SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_UNSUPPORTED_FILTER_OPERATION, operation);
                    }
                }
            } else {
                throw handleClientException(
                        SecurityConstants.KeyStoreMgtConstants.ErrorMessage.ERROR_CODE_BAD_VALUE_FOR_FILTER, filter);
            }
        }
        return aliases;
    }
    ...
    private String getTrustStoreName() {
        ...
        String filePath = serverConfiguration.getFirstProperty(
                SecurityConstants.KeyStoreMgtConstants.SERVER_TRUSTSTORE_FILE);
